...
def get_base_ydl_opts():
    ...
    opts = {
        ...
        'compat_opts': ['no-direct-merge', 'no-keep-subs', 'no-live-chat'],
        'overwrites': True,
        'no_resume': True,
        'ignoreerrors': True,
        'extract_flat': False,
...
def get_info():
    ...
    try:
        ...
        if not info:
            ...
            try:
                ydl_opts_fallback = get_base_ydl_opts()
                ydl_opts_fallback['skip_download'] = True
                ydl_opts_fallback['extractor'] = 'generic'
                ydl_opts_fallback['force_generic_extractor'] = True
                
                with yt_dlp.YoutubeDL(ydl_opts_fallback) as ydl:
                    info = ydl.extract_info(url, download=False)
                
                if not info:
                    return jsonify({'error': 'This URL is not supported or the video is not accessible. Please try a different URL.'}), 400
            except Exception as generic_error:
                return jsonify({'error': f'Could not extract video information. The video may be private, geo-blocked, or the URL is not supported. Error: {str(generic_error)}'}), 400
        ...
        try:
            ...
            if not info:
                return jsonify({'error': 'Could not extract video information with generic extractor.'}), 400
            ...
            return jsonify({'error': f'Extractor error: The video structure has changed. Try a different video or update yt-dlp. Details: {str(e)}'}), 400
...
def download_video():
    ...
    try:
        ...
        if not downloaded_file or not os.path.exists(downloaded_file):
            ...
            try:
                ydl_opts_simple = get_base_ydl_opts()
                ydl_opts_simple.update({
                    'format': 'best',
                    'outtmpl': output_template,
                    'overwrites': True,
                    'no_resume': True
                ...
                if not downloaded_file or not os.path.exists(downloaded_file):
                    return jsonify({'error': 'Download failed — file not found after extraction. The video may not be available or the format is not supported.'}), 500
            except Exception as simple_error:
                return jsonify({'error': f'Download failed with both complex and simple formats. Error: {str(simple_error)}'}), 500
        ...
        if "not made this video available in your country" in err_msg:
            ...
            return jsonify({'error': 'YouTube detected automated access. Please update your cookies.txt file.'}), 400
        elif "HTTP Error 403" in err_msg or "HTTP Error 429" in err_msg:
            return jsonify({'error': 'The video platform is blocking downloads. Try again later or use a different video.'}), 400
